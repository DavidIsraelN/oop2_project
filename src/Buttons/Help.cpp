...
//----------------------------------------------------------
Help::Help(float width, float height, const sf::Vector2f& size, const sf::Vector2f& position, const sf::Vector2f& window_size)
...
{
  m_help.setTexture(ResourceManager::Resource().getTexture(TextureIndex::HELP));
  m_help.setScale(window_size.x / m_help.getGlobalBounds().width, window_size.y / m_help.getGlobalBounds().height);
}
