...
void Level::setLevel(Action level)
{
  ...
  m_current_board = std::make_unique<LevelReader>(
    ResourceManager::Resource().getTxtFile(TxtIndex(level)), m_win_width, m_win_height);
  m_current_board->getLevel(m_player, m_balls, m_walls, m_doors);

  m_background = sf::Sprite(ResourceManager::Resource().getBackgroundTexture(BackgroundIndex(level)));
  m_background.setScale(m_current_board->getWorldWidth() / m_background.getGlobalBounds().width,
...
void Level::resetLevel()
{
  ...
  m_bullets.clear();
  m_balls->clear();
  m_current_board->getOriginalBalls(m_balls);
  std::for_each(m_balls->begin(), m_balls->end(), [](auto &ball){ball->setOriginalPosition();});
}
  ...
  for (auto i = size_t(0); i < m_balls->size(); ++i)
    if ((*m_balls)[i]->isDel() && (*m_balls)[i]->getRatio() > 1)
    {
      m_balls->emplace_back(std::make_shared<Ball>((*m_balls)[i]->getRatio() - 1,
        ...
        m_win_height - m_current_board->getObjHeight(), 1));
      m_balls->emplace_back(std::make_shared<Ball>((*m_balls)[i]->getRatio() - 1,
        m_current_board->getObjWidth(),
        sf::Vector2f((*m_balls)[i]->getGlobalBounds().left + (*m_balls)[i]->getGlobalBounds().width / 2,
          (*m_balls)[i]->getGlobalBounds().top + (*m_balls)[i]->getGlobalBounds().height / 2),
...
float Level::getFirstDoor() const
{
  ...
  return m_doors->empty() ? m_current_board->getWorldWidth() :
...
{
  return m_player->getGlobalBounds().left > m_current_board->getWorldWidth();
}
